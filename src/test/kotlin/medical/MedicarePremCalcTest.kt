...
class MedicarePremCalcTest : FunSpec({
    ...
    val brackets = listOf(
        MedicarePremBracketRec(
            ...
            partPrems = MedicarePartPrems(partBPrem = 2000.0, partDPrem = 500.0)
        ),
        MedicarePremBracketRec(
            singleBracket = MedicareBracket(start = 10000.0, end = 20000.0),
            jointBracket = MedicareBracket(start = 20000.0, end = 40000.0),
            partPrems = MedicarePartPrems(partBPrem = 2200.0, partDPrem = 600.0)
        ),
        MedicarePremBracketRec(
            ...
            partPrems = MedicarePartPrems(partBPrem = 2400.0, partDPrem = 700.0)
        ),
    )
    ...
    test("getPremium calculated premium when filing method is single and agi (adjusted for inflation) is in first bracket") {
        ...
        val results = calculator.getMedicarePremium(currYear, agi)
        results.shouldBe(brackets[0].partPrems.partBPrem * cmpdMedInfl +
            brackets[0].partPrems.partDPrem * cmpdMedInfl)
    }

    test("getPremium calculated premium when filing method is head of household and agi (adjusted for inflation) is in first bracket") {
        val agi = (brackets[0].singleBracket.end - 1.0) * cmpdStdInfl
    ...
    test("getPremium calculated premium when filing method is jointly and agi (adjusted for inflation) is in first bracket") {
    ...
    test("getPremium calculated premium when filing method is single and agi (adjusted for inflation) is in second bracket") {
        val agi = (brackets[1].singleBracket.end - 1.0) * cmpdStdInfl
        val currYear = yearlyDetailFixture(year, inflation, filingStatus = FilingStatus.SINGLE)
        val results = calculator.getMedicarePremium(currYear, agi)
        results.shouldBe(brackets[1].partPrems.partBPrem * cmpdMedInfl +
            brackets[1].partPrems.partDPrem * cmpdMedInfl)
    }

    test("getPremium calculated premium when filing method is jointly and agi is in the last bracket") {
        val agi = 5000000.0
        val currYear = yearlyDetailFixture(year, inflation, filingStatus = FilingStatus.JOINTLY)
        val results = calculator.getMedicarePremium(currYear, agi)
        results.shouldBe(brackets[2].partPrems.partBPrem * cmpdMedInfl +
